...
IKO_LEVELS = ['Asystent', 'Instruktor 1', 'Instruktor 2', 'Insturktor 3']
...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        self.create_instructors(
            10
        )  # CHANGE userType in instructor.models.save() to 'INSTRUCTOR'
            ...
            for day in range(1, 2):
                for instructor in instructor_ids:
                    for lesson_per_day in range(random.randint(1, 7)):
                        ...
                        lesson_start_hours.pop(
                            lesson_start_hours.index(_unique_start_hour)
                        )
                        ...
                        confirmed = random.choice([True, True, True, True, False])
                        in_progress = random.choice(
                            [
                                bool(confirmed),
                                bool(confirmed),
                                bool(confirmed),
                                bool(confirmed and False),
                            ]
                        )
                        ...
                        if lesson.completed:
                            ...
                            for student in lesson.student.all():
                                ...
                                lesson_detail = LessonDetail.objects.create(
                                    ...
                                    if is_group_lesson
                                    else student.pay_rate_single,
                                    price=duration
                                    * int(
                                        student.pay_rate_group
                                        if is_group_lesson
                                        else student.pay_rate_single
                                    ),
